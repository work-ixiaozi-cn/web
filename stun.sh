...
# Step 2: 获取stun列表
response=$(curl -s -X GET -H "Content-Type: application/json" -H "Authorization: $token" "http://192.168.1.11:16601/api/stunrulelist?_=$(date +%s)" | sed 's/\\\"/'\''/g' | sed 's/"LogContent":".*","LogTime"/"LogContent":"","LogTime"/')
if [ -z "$response" ]; then
